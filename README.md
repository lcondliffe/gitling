...
gitling churn --since 1y # focused file-churn drill-down (all files, ranked)
...
activity view with day/week/month buckets, and `gitling churn` / `--churn`
ranks every file touched in range by commit count. The remaining drill-downs
(`--contributors`, `--branches`) are reserved but not yet implemented.
